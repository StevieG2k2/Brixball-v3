...
function drawIt() {
	...
	function hiscore(){
		const rekordi = JSON.parse(localStorage.getItem(HIGH_SCORES)) || [];
		const lowestScore = rekordi[NO_OF_HIGH_SCORES - 1]?.sc ?? 0;
		var flag = false

		if (tocke >= lowestScore) {
			document.getElementById("mozak").style.visibility = "visible";
			input.addEventListener("keypress", function(event) {
				if (event.key === "Enter") {
					event.preventDefault();
					dodajRekord(tocke, rekordi);
					document.getElementById("mozak").style.visibility = "hidden";
					flag = true
				}
			});
	...
	function dodajRekord(sc, rekordi) {
		const d = new Date()
		const datum = d.getDate()+"."+(d.getMonth()+1)+"."+d.getFullYear()
		var mwz = document.getElementById("ime").value;
		if(mwz == null || mwz == ""){
			mwz = "gost"
		}else{
			mwz = mwz.substring(0,12)
		}
		if(minuteI == undefined){
			minuteI = "00"
		}
		if(sekundeI == undefined){
			sekundeI = "00"
		}
		const newScore = { 
			sc,
		...
		// 1. Add to list
		rekordi.push(newScore);

		// 2. Sort the list
		rekordi.sort((a, b) => a.minuteI - b.minuteI);
		rekordi.sort((a, b) => a.sekundeI - b.sekundeI);
		rekordi.sort((a, b) => b.sc - a.sc);
		
		// 3. Select new list
		rekordi.splice(NO_OF_HIGH_SCORES);
		
		// 4. Save to local storage
		localStorage.setItem(HIGH_SCORES, JSON.stringify(rekordi));
	}
	//########################################################################################################################################################
